...
const dictionaries = {
  en: {
    ...
    checkHid: 'If Connect fails: close Chrome fully and start it with --disable-features=WebHidRecursiveFiltering',
    emptyPicker: 'Empty HID list. Chrome hides the keyboard-class control interface of some Razer mice. Restart Chrome with --disable-features=WebHidRecursiveFiltering',
    hidNeedOtherInterface: 'Chrome hid the DeathAdder control interface (it is a keyboard-class HID) and only exposed the pointer. Close Chrome and launch: chrome.exe --disable-features=WebHidRecursiveFiltering{hint}',
    support: 'Support',
  ...
  ru: {
    ...
    checkHid: 'Если Connect не проходит: полностью закрой Chrome и запусти chrome.exe --disable-features=WebHidRecursiveFiltering',
    support: 'Поддержка',
    ...
    emptyPicker: 'Список HID пустой. Chrome прячет клавиатурный управляющий интерфейс части мышей Razer. Перезапусти Chrome с --disable-features=WebHidRecursiveFiltering',
    ...
    hidNeedOtherInterface: 'Chrome спрятал управляющий интерфейс DeathAdder (он как клавиатура) и отдал только указатель. Закрой Chrome и запусти: chrome.exe --disable-features=WebHidRecursiveFiltering{hint}',
    lang: 'Язык',
  },
};
